...
&emsp; [3.1. `SiNPle`'s own tabular output](#31-sinples-own-tabular-output)<br>
&emsp; [3.2. VCF output](#32-vcf-output)<br>
...
The input is either the pileup format produced by commands such as `samtools mpileup`, or, with `--input-format gem`, the [GEM mapper](https://github.com/smarco/gem3-mapper)'s own output. The output is either a VCF, `SiNPle`'s own table with `--output-format sinple`, or both. A VCF to standard output is thus the shortest invocation:
...
### 3.1. `SiNPle`'s own tabular output
...
```
This is SiNPle version 1.2.1-159 [22-Sep-2026]
